  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-slate-50 via-blue-50/30 to-indigo-50/40 relative overflow-hidden">
      ...
      <div className="absolute inset-0 overflow-hidden pointer-events-none">
        ...
        <div className="absolute top-80 right-20 w-96 h-96 bg-gradient-to-br from-indigo-200/20 to-cyan-200/20 rounded-full blur-3xl animate-pulse" style={{ animationDelay: '1s' }}></div>
        <div className="absolute bottom-40 left-1/4 w-80 h-80 bg-gradient-to-br from-purple-200/15 to-pink-200/15 rounded-full blur-3xl animate-pulse" style={{ animationDelay: '2s' }}></div>
        <div className="absolute top-1/2 right-1/4 w-64 h-64 bg-gradient-to-br from-emerald-200/15 to-teal-200/15 rounded-full blur-3xl animate-pulse" style={{ animationDelay: '3s' }}></div>
      </div>
      ...
      <section className="relative min-h-screen flex items-center justify-center px-4 sm:px-6 py-12">
        ...
        <div className={`relative z-10 text-center transition-all duration-1000 ${isVisible ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-10'}`}>
          ...
          <div className="flex flex-col sm:flex-row flex-wrap justify-center gap-3 sm:gap-4 mb-8 px-4">
            <Button variant="outline" size="lg" className="group hover:bg-blue-600 hover:text-white transition-all duration-300 border-blue-200 text-sm sm:text-base">
          ...
          <div className="grid gap-6 sm:gap-8 md:grid-cols-2">
            <Card className="hover:shadow-2xl transition-all duration-300 hover:-translate-y-2 border-0 shadow-xl bg-white/95 backdrop-blur-sm hover:bg-white group">
              <CardHeader className="pb-4">
                <CardTitle className="text-gray-900 text-lg sm:text-xl group-hover:text-emerald-600 transition-colors">Abdul Kalam Technical University</CardTitle>
                <CardDescription className="text-base sm:text-lg text-gray-600">Bachelor of Technology in Computer Science</CardDescription>
              </CardHeader>
              <CardContent>
                <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-2">
                  <span className="text-gray-500 font-medium">2023 - 2026</span>
                  <Badge className="bg-green-100 text-green-800 hover:bg-green-200 text-xs sm:text-sm">Current CGPA: 9.6/10</Badge>
                </div>
              </CardContent>
            </Card>
            <Card className="hover:shadow-2xl transition-all duration-300 hover:-translate-y-2 border-0 shadow-xl bg-white/95 backdrop-blur-sm hover:bg-white group">
              <CardHeader className="pb-4">
                <CardTitle className="text-gray-900 text-lg sm:text-xl group-hover:text-emerald-600 transition-colors">Board of Technical Education Uttar Pradesh</CardTitle>
                <CardDescription className="text-base sm:text-lg text-gray-600">Polytechnic Diploma in Computer Science</CardDescription>
              </CardHeader>
              <CardContent>
                <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-2">
                  <span className="text-gray-500 font-medium">2020 - 2023</span>
                  <Badge className="bg-blue-100 text-blue-800 hover:bg-blue-200 text-xs sm:text-sm">CGPA: 8.1/10</Badge>
                </div>
              </CardContent>
            </Card>
          </div>
        ...
        <div className="max-w-6xl mx-auto relative">
          ...
          <div className="grid gap-4 sm:gap-6 grid-cols-1 sm:grid-cols-2 lg:grid-cols-4">
            {Object.entries(skills).map(([category, items], index) => (
              <Card key={category} className="hover:shadow-2xl transition-all duration-300 hover:-translate-y-2 border-0 shadow-xl bg-white/95 backdrop-blur-sm hover:bg-white group animate-fade-in" style={{ animationDelay: `${index * 100}ms` }}>
                <CardHeader className="pb-3 sm:pb-4">
                  <CardTitle className="capitalize text-gray-900 text-base sm:text-lg group-hover:text-blue-600 transition-colors">{category.replace(/([A-Z])/g, ' $1').trim()}</CardTitle>
                </CardHeader>
                <CardContent>
                ...
                <CardHeader className="pb-3 sm:pb-4">
                  <div className="flex flex-col gap-3 sm:gap-4 md:flex-row md:justify-between md:items-start">
                    <div className="flex-1">
                      <CardTitle className="text-gray-900 text-lg sm:text-xl mb-2 group-hover:text-purple-600 transition-colors">{exp.title}</CardTitle>
                    </div>
                    <Badge variant="outline" className="bg-purple-50 text-purple-700 border-purple-200 self-start text-xs sm:text-sm">{exp.duration}</Badge>
                  </div>
                </CardHeader>
                <CardContent>
                  <ul className="space-y-2 sm:space-y-3">
                    {exp.achievements.map((achievement, i) => (
                      <li key={i} className="flex items-start gap-3 text-gray-600 leading-relaxed text-sm sm:text-base">
        ...
        <div className="max-w-6xl mx-auto relative">
          ...
          <div className="grid gap-6 sm:gap-8 md:grid-cols-2">
            {projects.map((project, index) => (
              <Card key={index} className="hover:shadow-2xl transition-all duration-300 hover:-translate-y-2 group border-0 shadow-xl bg-white/95 backdrop-blur-sm hover:bg-white">
                <CardHeader className="pb-3 sm:pb-4">
      ...
      <footer className="py-12 sm:py-16 px-4 sm:px-6 bg-gradient-to-r from-gray-900 via-blue-900 to-purple-900 text-white relative overflow-hidden">
        ...
        <div className="max-w-6xl mx-auto text-center relative">
          <h3 className="text-xl sm:text-2xl md:text-3xl font-bold mb-4">Let's Build Something Amazing Together</h3>
          <p className="text-gray-300 mb-6 sm:mb-8 text-base sm:text-lg px-4">Always open to discussing new opportunities and innovative projects</p>
          <div className="flex justify-center">
            <Button className="bg-white text-gray-900 hover:bg-gray-100 transition-colors shadow-lg hover:shadow-xl transform hover:scale-105 transition-transform">
